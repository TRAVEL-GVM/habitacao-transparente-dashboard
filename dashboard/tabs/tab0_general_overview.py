...
def show_visao_geral_tab(df):
    ...
    living_with_others_pct = (df['housing_situation'] == 'Living with others').mean() * 100
    ...
    st.markdown("""
    <div class="header-title">Visão Geral do Habitação em Portugal</div>
    """, unsafe_allow_html=True)
    
    # Join the two text blocks with the styling from the satisfaction text
    st.markdown("""
    <p style="font-size: 16px; color: #1b5e20; text-align: center; margin-bottom: 20px; 
              background-color: #e8f5e9; padding: 10px; border-radius: 5px; border-left: 4px solid #2e7d32;">
    ...
    # Housing Affordability Simulator with enhanced styling
    st.markdown("""
    <h2 style="color: #1b5e20; border-bottom: 2px solid #2e7d32; padding-bottom: 10px;">
        Simulador de Acessibilidade Habitacional
    </h2>
    <div style="background-color: #f1f8e9; padding: 15px; border-radius: 8px; margin-bottom: 20px; border-left: 4px solid #66bb6a;">
    ...
    with col1:
        st.markdown(f"""
        <div class="metric-card" style="background-color: #f1f8e9;">
    ...
    with col2:
        ...
        <div class="metric-card" style="background-color: #e8f5e9;">
            ...
            <div style="color: #388e3c; font-size: 12px; margin-top: 5px;">30% do rendimento</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    st.markdown("""
    <h2 style="color: #1b5e20; border-bottom: 2px solid #2e7d32; padding-bottom: 10px;">
        Análise Geográfica e Indicadores
    </h2>
    """, unsafe_allow_html=True)
    ...
    with map_col:
        ...
        district_satisfaction = df.groupby('distrito')['satisfaction_score'].agg(['mean', 'count']).reset_index()
        ...
        try:
            ...
            # Create map centered on Portugal with better styling
            m = folium.Map(
                location=[39.6, -8.0],
                zoom_start=6,
            ...
            for feature in portugal_geojson["features"]:
                ...
                if district_name in district_satisfaction_dict:
                    ...
                    # Create custom popup HTML with better styling
                    def create_popup_html(district_name, score, count):
                        ...
                        <div style="font-family: Arial, sans-serif; padding: 2px; ">
                            ...
                            <div style="margin-top: 10px; text-align: center; font-style: italic;">
                                Use a caixa de seleção abaixo do mapa para filtrar por distrito
                            </div>
                        </div>
                        """
                        return html
        ...
        st.info("Para ver detalhes por distrito, passe o rato sobre o mapa e selecione um distrito na caixa abaixo."
        "Para mais detalhes, clique no botão de ecrã inteiro no canto superior esquerdo do mapa.")
    ...
    with kpi_col:
        ...
        if st.session_state.selected_district and st.session_state.selected_district != "All":
            filtered_df = df[df["distrito"].str.lower() == st.session_state.selected_district.lower()]
        ...
        avg_rent = filtered_df[filtered_df['housing_situation'] == 'Renting']['valor-mensal-renda'].mean()
        avg_purchase = filtered_df[filtered_df['housing_situation'] == 'Owned']['valor-compra'].mean()
        
        # Rent burden percentage
        rent_burden_data = filtered_df[filtered_df['housing_situation'] == 'Renting']
        high_burden_pct = rent_burden_data[rent_burden_data['rent_burden'].isin(['51-80% (High)', '>80% (Very High)'])].shape[0] / rent_burden_data.shape[0] * 100 if not rent_burden_data.empty else 0
        ...
        burden_severity = "alta" if high_burden_pct > 40 else "moderada" if high_burden_pct > 20 else "baixa"
        satisfaction_level = "alta" if avg_satisfaction >= 4 else "moderada" if avg_satisfaction >= 3 else "baixa"
        
        # Format rent and purchase prices with thousands separator
        avg_rent_formatted = f"{avg_rent:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
        avg_purchase_formatted = f"{avg_purchase:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
